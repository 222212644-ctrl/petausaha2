...
export default function ExportTabel() {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(business =>
        ...
        (business.sumber_data && business.sumber_data.toLowerCase().includes(q))
      );
    }
  ...
  const isFilterDisabled = (key: keyof Filters) => {
    if (filters.kecamatan === 'all' && key !== 'kecamatan') return true;
          ...
          <CardContent className={`${filterOpen ? 'block' : 'hidden md:block'}`}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-4">
              ...
              <Select 
                ...
                disabled={isFilterDisabled('jenis_peta')}
              >
                ...
                <TableBody>
                  {currentBusinesses.map((business, index) => (
                    <TableRow key={business.id}>
                      ...
                      <TableCell>
                        <Badge className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getJenisPetaColor(business.jenis_peta)}`}>
                          {business.jenis_peta}
                        </Badge>
                      </TableCell>
