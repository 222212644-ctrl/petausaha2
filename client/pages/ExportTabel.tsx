...
interface Filters {
  ...
  kecamatan: string;
  kelurahan_desa: string;
  // area_unit: choose which field to use for area_value
...
export default function ExportTabel() {
  ...
  const [filters, setFilters] = useState<Filters>({
    ...
    kecamatan: 'all',
    kelurahan_desa: 'all',
    // default area unit + value (mirip PetaUsaha)
    area_unit: 'sls',
    area_value: 'all',
    sumber_data: 'all'
  });
  ...
  useEffect(() => {
    const loadBusinesses = async () => {
      setLoading(true);
      try {
        const res = await fetch('/geojson/businesses-medan.geojson');
        ...
        if (data && Array.isArray(data.features)) {
          // 👉 GeoJSON standar
          businessData = data.features.map((feature: any) => {
          ...
          businessData = rows.slice(0, n).map((row, idx) => {
            const obj: any = {};
            headers.forEach((h, i) => {
              obj[h] = row[i];
            });
            const [lon, lat] = data.geometry[idx];
            obj.coordinates = [lat, lon] as [number, number];
            return obj;
          });
        } else {
          throw new Error('Format file bisnis tidak dikenali');
        }
  ...
  useEffect(() => {
    ...
    // apply area filter (area_value) using selected area_unit
  ...
  const updateFilter = (key: keyof Filters, value: string) => {
    setFilters(prev => {
      const newFilters = { ...prev, [key]: value } as Filters;
      ...
      // enforce constraints when parent is set back to 'all'
  ...
  const isFilterDisabled = (key: keyof Filters) => {
    // Allow selecting specific children even if parent is 'all', except:
    // - kelurahan disabled when kecamatan === 'all' (user requested)
    // - area_unit & area_value disabled when kelurahan === 'all' (user requested)
    if (key === 'kelurahan_desa' && filters.kecamatan === 'all') return true;
    if ((key === 'area_unit' || key === 'area_value') && filters.kelurahan_desa === 'all') return true;
    return false;
  };
  ...
  // New: return unique values for a field but scoped by current filters (hierarchical)
  const getFilteredOptions = (field: keyof Business, dependsOn?: keyof Filters): string[] => {
    let data = businesses.slice();

    if (dependsOn) {
      // apply hierarchical constraints up to the dependent level
      ...
      if (filters.kelurahan_desa !== 'all' && (field === (filters.area_unit === 'sls' ? 'sls' : 'blok_sensus') || field === 'kelurahan_desa')) {
        data = data.filter(b => b.kelurahan_desa === filters.kelurahan_desa);
      }
    }

    const vals = data.map(b => String(b[field])).filter(Boolean);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-4">
              ...
              >
                ...
                <SelectContent className="max-h-56 overflow-y-auto transition-all duration-150">
                  ...
                  {getFilteredOptions('jenis_peta').map(jenis => (
          ...
          <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center justify-center space-x-2">
              ...
              <div className="flex items-center space-x-1">
                {(() => {
                  // window of up to 5 pages, centered on currentPage when possible
                  const maxButtons = Math.min(5, totalPages);
                  const half = Math.floor(maxButtons / 2);
                  let startPage = Math.max(1, currentPage - half);
                  // ensure we have maxButtons pages when near the end
